...
4. Let's also add Angular Material so we can use some of their components: `yarn add @material-ui/core`
5. Use the [`@nrwl/react` plugin](https://nx.dev/react/api/react/schematics/application) to generate an React app called `store` in your new workspace
